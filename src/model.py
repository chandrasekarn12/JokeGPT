...
class GPTConfig:
    def __init__(self, vocab_size, block_size, n_layers, n_heads, n_embd, dropout, start_token=0):
...
class GPTLanguageModel(nn.Module):
    ...
    def generate(self, idx=None, max_new_tokens=100, temperature=1.0):
        if idx is None or idx.numel() == 0:
            start_token = self.config.start_token
            idx = torch.tensor([[start_token]], dtype=torch.long).to(next(self.parameters()).device)
